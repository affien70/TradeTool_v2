...
def render() -> None:
    ...
    if st.button('Kjør candidate-type diagnostikk'):
        if not database_path.strip():
            st.warning('Oppgi en lokal databasebane for å kjøre candidate-type-diagnostikk.')
            return
        try:
            ...
            st.error(str(exc))
            return
        st.subheader('Candidate-type diagnostikk')
        st.caption('Dette er bare en diagnostisk kandidatklassifisering, ikke kjøpsråd eller produksjonsgodkjenning.')
        st.json(result.to_summary_dict())
        st.dataframe(
            [
                {
                    ...
                    'classification_reasons': ', '.join(row.classification_reasons),
                    'classification_warnings': ', '.join(row.classification_warnings),
                }
                for row in result.rows[:20]
            ],
            use_container_width=True,
