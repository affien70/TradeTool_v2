...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description='Compare the no-drawdown challenger against the v2 incumbent baseline.')
    ...
    parser.add_argument('--out-dir', required=True, help='Output directory for no-drawdown challenger comparison report files.')
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = build_challenger_no_drawdown_comparison(**kwargs)
    out_dir = Path(args.out_dir).expanduser().resolve()
    write_challenger_no_drawdown_comparison_outputs(result=result, out_dir=out_dir)
    print(f'{result.decision_recommendation} no-drawdown challenger comparison written to {out_dir}')
    return 0
