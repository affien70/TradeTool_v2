...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description='Run a bounded monthly holdout forward-return diagnostic panel.')
    ...
    parser.add_argument('--out-dir', required=True, help='Output directory for monthly holdout report files.')
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = build_monthly_holdout_backtest(**kwargs)
    out_dir = Path(args.out_dir).expanduser().resolve()
    write_monthly_holdout_outputs(result=result, out_dir=out_dir)
    print(f'{result.decision_recommendation} monthly holdout backtest written to {out_dir}')
    return 0
