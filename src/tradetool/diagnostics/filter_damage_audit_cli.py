...
from tradetool.diagnostics.filter_damage_audit import build_filter_damage_audit, write_filter_damage_audit_outputs


def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description='Audit which challenger filters removed incumbent baseline picks.')
    ...
    parser.add_argument('--out-dir', required=True, help='Output directory for filter damage audit report files.')
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = build_filter_damage_audit(**kwargs)
    out_dir = Path(args.out_dir).expanduser().resolve()
    write_filter_damage_audit_outputs(result=result, out_dir=out_dir)
    print(f'{result.decision_recommendation} filter damage audit written to {out_dir}')
    return 0
