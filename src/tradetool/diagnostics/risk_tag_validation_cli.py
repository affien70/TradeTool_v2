...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description='Validate incumbent screener risk tags against forward returns.')
    ...
    parser.add_argument('--out-dir', required=True, help='Output directory for risk tag validation report files.')
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = build_risk_tag_validation(**kwargs)
    out_dir = Path(args.out_dir).expanduser().resolve()
    write_risk_tag_validation_outputs(result=result, out_dir=out_dir)
    print(f'{result.decision_recommendation} risk tag validation written to {out_dir}')
    return 0
