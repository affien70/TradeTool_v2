...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description='Calculate forward returns for one historical holdout snapshot.')
    ...
    parser.add_argument('--out-dir', required=True, help='Output directory for forward-return report files.')
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = build_holdout_forward_returns(**kwargs)
    out_dir = Path(args.out_dir).expanduser().resolve()
    write_holdout_forward_return_outputs(result=result, out_dir=out_dir)
    print(f'{result.decision_recommendation} holdout forward returns written to {out_dir}')
    return 0
