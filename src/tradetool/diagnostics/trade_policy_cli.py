...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description='Generate read-only trade-policy diagnostics from a SQLite copy.')
    parser.add_argument('--db-path', required=True, help='Path to a local SQLite copy.')
    parser.add_argument('--universe-id', required=True, help='Universe identifier for the trade-policy report.')
    parser.add_argument('--benchmark-ticker', help='Optional benchmark ticker.')
    parser.add_argument('--out-dir', required=True, help='Output directory for the generated trade-policy files.')
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f'VALID trade policy diagnostics written to {out_dir}')
    return 0
