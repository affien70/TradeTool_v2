...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description='Compare v2 monthly holdout groups against naive baselines without tuning.')
    ...
    parser.add_argument('--out-dir', required=True, help='Output directory for baseline/naive comparison report files.')
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = build_baseline_naive_comparison(**kwargs)
    out_dir = Path(args.out_dir).expanduser().resolve()
    write_baseline_naive_comparison_outputs(result=result, out_dir=out_dir)
    print(f'{result.decision_recommendation} baseline naive comparison written to {out_dir}')
    return 0
