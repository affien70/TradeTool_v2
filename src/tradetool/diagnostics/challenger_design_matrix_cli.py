...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description='Run the research-only challenger design matrix for one universe.')
    ...
    parser.add_argument('--out-dir', required=True, help='Output directory for challenger design matrix report files.')
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = build_challenger_design_matrix(**kwargs)
    out_dir = Path(args.out_dir).expanduser().resolve()
    write_challenger_design_matrix_outputs(result=result, out_dir=out_dir)
    print(f'{result.decision_recommendation} challenger design matrix written to {out_dir}')
    return 0
