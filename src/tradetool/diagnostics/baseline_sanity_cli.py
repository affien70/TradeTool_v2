...
from tradetool.diagnostics.baseline_sanity import build_baseline_sanity_diagnostics, write_baseline_sanity_outputs


def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description='Generate read-only baseline sanity diagnostics from a SQLite copy.')
    parser.add_argument('--db-path', required=True, help='Path to a local SQLite copy.')
    parser.add_argument('--universe-id', required=True, help='Universe identifier for the baseline sanity report.')
    parser.add_argument('--benchmark-ticker', help='Optional benchmark ticker.')
    parser.add_argument('--evidence-dir', required=True, help='Copied Phase 1 evidence directory.')
    parser.add_argument('--out-dir', required=True, help='Output directory for the generated baseline sanity files.')
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f'VALID baseline sanity diagnostics written to {out_dir}')
    return 0
