...
from tradetool.diagnostics.challenger_comparison import CHALLENGER_ID as FAILED_CHALLENGER_ID
from tradetool.diagnostics.challenger_no_drawdown_comparison import (
    CHALLENGER_ID,
    DECISION_FIX_METHODOLOGY,
    DECISION_INVESTIGATE_FILTER_DAMAGE,
    DECISION_KEEP_INCUMBENT,
    DECISION_PROMOTE,
    EXPECTED_REPORT_FILES,
    GROUP_FAILED_CHALLENGER,
    GROUP_INCUMBENT,
    GROUP_NO_DRAWDOWN_CHALLENGER,
    build_challenger_no_drawdown_comparison,
    select_challenger_rs6m_trend_liquidity_no_drawdown,
    write_challenger_no_drawdown_comparison_outputs,
)
from tradetool.diagnostics.challenger_no_drawdown_comparison_cli import build_argument_parser
from tradetool.diagnostics.challenger_no_drawdown_comparison_cli import main as no_drawdown_cli_main
...
def _rows(*, no_drawdown_wins: bool = True, remaining_filter_damage: bool = False) -> tuple[dict[str, object], ...]:
    rows: list[dict[str, object]] = []
    for rank in range(1, 31):
        incumbent_member = rank <= 10
        no_drawdown_member = 11 <= rank <= 20
        if no_drawdown_wins:
            excess20 = 0.05 if no_drawdown_member else 0.02
            excess60 = 0.14 if no_drawdown_member else 0.04
        elif remaining_filter_damage:
            excess20 = 0.05 if incumbent_member else (0.02 if no_drawdown_member else -0.01)
            excess60 = 0.15 if incumbent_member else (0.03 if no_drawdown_member else -0.02)
        else:
            excess20 = 0.02 if no_drawdown_member else 0.05
            excess60 = 0.03 if no_drawdown_member else 0.12
        rows.append(
            _candidate(
                rank=rank,
                rs6=300.0 - rank if incumbent_member else 100.0 - rank,
                rs3=500.0 - rank if no_drawdown_member else float(rank),
                return6=0.5 if no_drawdown_member else (-0.1 if incumbent_member else 0.2),
                above_sma200=(not incumbent_member) if remaining_filter_damage else True,
                traded_value=1_000_000.0,
                drawdown=-0.80 if incumbent_member or (remaining_filter_damage and no_drawdown_member) else -0.10,
                stretch=0.10,
                excess20=excess20,
                excess60=excess60,
            )
...
class ChallengerNoDrawdownComparisonTests(unittest.TestCase):
    def setUp(self) -> None:
        self.db_path = Path('/tmp/tradetool_challenger_no_drawdown_readonly.sqlite')
        self.out_dir = Path('/tmp/tradetool_challenger_no_drawdown_output')
        self._cleanup()
    ...
    def _build(self, *, rows: tuple[dict[str, object]] | None = None, invalid: bool = False):
        ...
        return build_challenger_no_drawdown_comparison(
    ...
    def test_no_drawdown_selection_ignores_drawdown_but_keeps_other_filters_and_sort(self) -> None:
        rows = (
            {'ticker': 'BBB.OL', 'above_sma200': True, 'return_6m': 0.2, 'relative_strength_6m': 0.5, 'relative_strength_3m': 0.3, 'average_traded_value_20': 1_000_000.0, 'drawdown_252': -0.95, 'distance_to_sma200': 0.1},
            {'ticker': 'AAA.OL', 'above_sma200': True, 'return_6m': 0.2, 'relative_strength_6m': 0.5, 'relative_strength_3m': 0.3, 'average_traded_value_20': 1_000_000.0, 'drawdown_252': -0.95, 'distance_to_sma200': 0.1},
        ...
        selected = select_challenger_rs6m_trend_liquidity_no_drawdown(rows, limit=3)
    ...
    def test_report_writes_expected_files_and_promote_decision(self) -> None:
        result = self._build()
        write_challenger_no_drawdown_comparison_outputs(result=result, out_dir=self.out_dir)
        self.assertEqual(sorted(path.name for path in self.out_dir.iterdir()), sorted(EXPECTED_REPORT_FILES))
        summary = json.loads((self.out_dir / 'challenger_no_drawdown_summary.json').read_text(encoding='utf-8'))
        self.assertEqual(summary['incumbent_id'], BASELINE_ID)
        self.assertEqual(summary['failed_challenger_id'], FAILED_CHALLENGER_ID)
        self.assertEqual(summary['challenger_id'], CHALLENGER_ID)
        self.assertEqual(summary['decision_recommendation'], DECISION_PROMOTE)

    def test_group_summary_includes_incumbent_failed_challenger_no_drawdown_and_context(self) -> None:
        result = self._build()
        groups = {row['group'] for row in result.group_summary_rows}
        self.assertIn(GROUP_INCUMBENT, groups)
        self.assertIn(GROUP_FAILED_CHALLENGER, groups)
        self.assertIn(GROUP_NO_DRAWDOWN_CHALLENGER, groups)
        self.assertIn('v2_raw_top10_context_only', groups)

    def test_overlap_and_removed_incumbent_rows_are_calculated(self) -> None:
        result = self._build(rows=_rows(no_drawdown_wins=False, remaining_filter_damage=True))
        overlap = next(
            row for row in result.overlap_rows
            if row['left_group'] == GROUP_INCUMBENT and row['right_group'] == GROUP_NO_DRAWDOWN_CHALLENGER and row['forward_window_trading_days'] == 60
        )
        self.assertEqual(overlap['overlap_count'], 0)
        self.assertIn('T01.OL', overlap['left_only_tickers'])
        removed = next(row for row in result.removed_vs_incumbent_rows if row['ticker'] == 'T01.OL')
        self.assertFalse(removed['included_in_no_drawdown_challenger'])
        self.assertIn('return_6m_positive', removed['remaining_filter_failures'])

    def test_keep_incumbent_decision_when_no_drawdown_does_not_improve_failed_challenger(self) -> None:
        rows = []
        for source in _rows(no_drawdown_wins=False):
            row = dict(source)
    ...
    def test_investigate_remaining_filter_damage_when_improved_but_still_loses(self) -> None:
        result = self._build(rows=_rows(no_drawdown_wins=False, remaining_filter_damage=True))
        self.assertEqual(result.decision_recommendation, DECISION_INVESTIGATE_FILTER_DAMAGE)
    ...
    def test_cli_requires_explicit_db_path_and_writes_outputs(self) -> None:
        ...
        import tradetool.diagnostics.challenger_no_drawdown_comparison_cli as cli_module
        original = cli_module.build_challenger_no_drawdown_comparison
        ...
        try:
            cli_module.build_challenger_no_drawdown_comparison = builder
            exit_code = no_drawdown_cli_main([
            ...
            cli_module.build_challenger_no_drawdown_comparison = original
    ...
    def test_no_production_ui_policy_ml_or_holdings_changes(self) -> None:
        ...
        source = Path('src/tradetool/diagnostics/challenger_no_drawdown_comparison.py').read_text(encoding='utf-8').lower()
